...
The estimated coefficient on tobacco is `r round(model2$coefficients[2], 2)` and the standard error is `r round(model2$std.error[2], 2)`.yt
...
Using the linear regression analogue, the average effect of a mother smoking on birth weight is a decrease of `r abs(round(model3[[1]][[2]], 2))` grams. The average effect of a mother smoking on birth weight is a decrease of `r round(abs(ATE), 2)` grams using the the exact matching estimator.
...
Using the propensity score, the estimated effect of a mother smoking on birth weight is a decrease of `r round(abs(wls[[1]][[2]]), 2)`.
...
Weighted refression is not that different from unweighted, why do we want to do this? Not having a big difference somewhat proves that this is reasonable

Two methods show that you re controlling for this OVB or if you are not.
